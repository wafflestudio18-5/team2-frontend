...
const ResponseInput = ({ showModal, logged_in, responseInput, setResponseInput, me, InputValue, setInputValue, postResponse }) => {
    if (!responseInput || !logged_in) {
        var clickinput;
        if (logged_in) clickinput = () => setResponseInput(true);
        else clickinput = () => showModal(ModalTypeConstants.LOG_IN);
        return (
            <ResponseInputWrapper>
                <ResponseInputBlock>
                    <ResponseInputInnerBlock onClick={clickinput}>
                        <ResponseInputWindow
                            data-gramm="false"
                            role="textbox"
                ...
                <ResponseInputBlockClicked>
                    <ResponseInputWriterBlock logged_in={logged_in}>
                        <ResponseInputWriterInfo>
                            <img src={profile} width="32" height="32" alt="img" />
